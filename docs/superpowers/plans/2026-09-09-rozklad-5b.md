...
- **Термінологія фіксована.** «Модуль» — предмет на 1,5 години (I–V). «Міні-модуль» — 30 хвилин усередині нього, їх три. У коді, у тестах і в UI — тільки ці слова. Слово «блок» не вживається на позначення 30-хвилинних відрізків.
...
Тиждень протилежного кольору зазвичай — наступний календарний, але `weekOverrides` може зсунути чергування так, що два сусідні тижні матимуть однаковий колір. Тому замість припущення «+7 днів» код шукає вперед (до 8 ітерацій) найближчий понеділок, чий реальний колір (через `weekColorFor`, який враховує `weekOverrides`) збігається з бажаним — `shownColor`, якщо він заданий, інакше колір поточного тижня. Показаний колір береться саме з цього знайденого тижня, а не з побажання, тож заголовок і вміст карток (які отримують уроки через `lessonsFor`, що теж рахує реальний колір) ніколи не розійдуться, навіть якщо потрібного кольору попереду не знайшлося за 8 тижнів. `color` теж не береться зі `state.color` — бо на вихідних і канікулах `state.color` дорівнює `null`, і екран «Тиждень» усе одно має показувати реальний тиждень. Дати в шапці й у картках завжди належать тому тижню, який показано, тож підпис не бреше. Якщо показаний тиждень припадає на канікули, `lessonsFor` поверне порожньо і картка чесно скаже «уроків немає».
